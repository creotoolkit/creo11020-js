...
*	History : 
* -----------------------------------------------------------
*  22-Apr-2019		P-70-07 	 rkumbhare   $$1 Created
*  11-Jun-2020          P-80-08      rkumbhare   $$2 Updated to add GlobalMethod in search menu.
*  22-Jun-2020          P-80-09      rkumbhare   $$3 Updated.
*
*
...
function createResults(results_entry) 
{
   ...
   for (var j = 0; j < results_entry.length; j++) {    
	...
	} else if (searchData[e][1][1][2] == "global"){
		apiName = searchData[e][1][0];
		apiLink = searchData[e][1][1][0];
	}else{
		if(searchData[e][0] != "") {
...
function getUrlParams () 
    {
        var vars_ = {};
		var vars = {};
        var searchkey = document.location.href.split("?");		
		if(searchkey != null) {		
			return searchkey[1];
		} else return null;
    }
    ...
    function parseUrlParams ()
    {
        ...
        if (params) {		
			var searchText = document.getElementById ("ApiSearchField");
			if (searchText) {
				...
				if ("createEvent" in document) {
					...
					searchText.dispatchEvent (evt);
				}
				else {
					searchText.fireEvent ("onkeyup");
				}
			}
        }
    }